...
- **Hybrid data model.** Comments are stored locally, not fetched live on
  every request. That's fine for an occasional check, but too slow if this
  were powering an internal dashboard hitting it constantly. Local data is
...
- **Async replies.** Posting a reply doesn't call the platform inline from
  ...
  queue like Kafka or SQS/Pub/Sub. That's right-sized for this system's
  current scale, not a permanent choice; a production system at real
  throughput would likely warrant one of those instead.

## Demo platform: GoToSocial

Real integrations with the likes of X, Instagram, or LinkedIn need OAuth
app registration, approval processes, and often webhook setup, none of
which is reasonable to stand up for a take-home. Instead, this repo
